...
// 수정하기 버튼 링크
$update_link_href = "adminuser_update.php?" . $QUERY_STRING . "&au_id=" . $_REQUEST["au_id"];
...
tp_set("color_au_usestate", $color_au_usestate_array[$row["au_usestate"]]);
tp_set("au_usestate", array_search($row["au_usestate"], $db_au_usestate_array));
...
foreach ($m_name_1_array as $key => $val) {
    ...
    foreach ($m_name_3_array[$key] as $key2 => $val2) {
        ...
        $m_name_3_list .= "&nbsp;[<span style=\"color:" . $color_m_name_3 . "\">" . $m_name_3_array[$key][$key2] . "</span>]&nbsp;";
    }
