...
$query .= "m_name ";
$query .= "from menu_tb ";
$query .= "where m_step = '" . $db_m_step_array["대메뉴"] . "' ";
...
// 등록하기 버튼 링크
$insert_link_href = "menu_insert.php?" . $QUERY_STRING;
...
while ($row = db_fetch_array($result)) {
    ...
    // 조회하기 링크
    $view_link_href_1 = "menu_view.php?" . $QUERY_STRING . "&m_id=" . $row["m_id"];
    
    // 순서변경 버튼
    if ($prev_m_id_1 != "") {
        $order_change_link_href_1 = "menu_order_change.php?" . $QUERY_STRING . "&m_id=" . $row["m_id"] . "&prev_m_id=" . $prev_m_id_1;
    ...
    while ($row_sub = db_fetch_array($result_sub)) {
        ...
        // 조회하기 링크
        $view_link_href_2 = "menu_view.php?" . $QUERY_STRING . "&m_id=" . $row_sub["m_id"];
        
        // 순서변경 버튼
        if ($prev_m_id_2 != "") {
            $order_change_link_href_2 = "menu_order_change.php?" . $QUERY_STRING . "&m_id=" . $row_sub["m_id"] . "&prev_m_id=" . $prev_m_id_2;
        ...
        while ($row_sub_sub = db_fetch_array($result_sub_sub)) {
            ...
            // 조회하기 링크
            $view_link_href_3 = "menu_view.php?" . $QUERY_STRING . "&m_id=" . $row_sub_sub["m_id"];
            
            // 순서변경 버튼
            if ($prev_m_id_3 != "") {
                $order_change_link_href_3 = "menu_order_change.php?" . $QUERY_STRING . "&m_id=" . $row_sub_sub["m_id"] . "&prev_m_id=" . $prev_m_id_3;
