...
// opener javascript
function opener_javascript($function_name) { 
    html_begin();
             
    echo "\n";
    echo "<script type=\"text/javascript\">\n";
    echo "/* <![CDATA[ */\n";
    echo "    opener." . $function_name . ";\n";
    echo "/* ]]> */\n";
    echo "</script>\n";
...
////////////////////////////////////////////////////////////////////////////////
// DB Date time function

// 숫자부분만 남김
function get_db_date_format($str) {
    if ($str == "") {
        return "";
    }
    else {
        $str = ereg_replace("[^0-9]", "", $str);            // 숫자만 남김 예) 2015-12-09 -> 20151209    
    }
...
function calc_begin_no($total_rows, $begin_row, $sort = "") {
    switch ($sort) {
        case "ascend":
            $begin_no = $begin_row;
            break;
...
// Create directory
function check_directory($pathname, $mode = 0777) {
    if (!is_dir($pathname)) {
